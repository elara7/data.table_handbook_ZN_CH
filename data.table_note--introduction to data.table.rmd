...
- 我们可以选择使某个变量满足某个具体条件的行，而且不需要在变量名前加上`DT$`来引用。
- 我们可以在`data.table[]`中使用`order()`进行高速排序

使用 j:

- 选择需要的列 `DT[, .(colA, colB)]`
- 如果想要变回data.frame的用法，仅仅享受加速，可以加入 `with = FALSE`，如 `DT[, c("colA", "colB"), with = FALSE]`
- 可以直接对列进行计算如 `DT[, .(sum(colA), mean(colB))]`
- 对列计算后的输出也可以命名如 `DT[, .(sA =sum(colA), mB = mean(colB))]`
- 和i参数结合使用 `DT[colA > value, sum(colB)]`

使用 by:

- 使用 `by` ，可以根据一个、一组列，甚至是表达式进行分组计算，并且可以和 `i`，`j` 参数灵活结合
- 使用 `keyby` 来得到按组排序后的结果
- 在 `j` 中使用 `.SD` 和 `.SDcols` 来对多列进行操作，例如

1. `DT[, lapply(.SD, fun), by = ..., .SDcols = ...]` 对 `.SDcols` 选定的列,根据 `by` 参数指定的列分组应用 `fun` 函数

2. `DT[, head(.SD, 2), by = ...]` 返回每一组的头2个

3. `DT[col > val, head(.SD, 1), by = ...]` `i` ,`j`, `by` 组合使用